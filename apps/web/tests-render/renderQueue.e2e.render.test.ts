...
import { buildPitchSceneV2 } from '@friendword/contracts';
...
function successScene() {
  const scene = buildPitchSceneV2({
    template: 'warm',
    photoAssetIds: [IDS.photo1, IDS.photo2],
    segments: [
...
describe.runIf(E2E)('render queue end to end (local Supabase + next start)', () => {
  ...
  beforeAll(async () => {
    ...
    psqlFile(`
BEGIN;
DELETE FROM ops_alerts WHERE campaign_id IN (${idList([IDS.campaignA, IDS.campaignB, IDS.campaignC])});
DELETE FROM campaigns WHERE id IN (${idList([IDS.campaignA, IDS.campaignB, IDS.campaignC])});
...
DELETE FROM pitch_drafts WHERE id IN (${idList([IDS.draftA, IDS.draftB, IDS.draftC])});
DELETE FROM users WHERE id IN (${idList([IDS.introducer, IDS.dater, IDS.daterB, IDS.daterC])});
...
INSERT INTO media_render_jobs (id, revision_id, campaign_id, pitch_draft_id, scene_hash, attempts, created_at)
SELECT 'ee3a4000-0000-4000-8000-000000000001', rev.id, '${IDS.campaignA}', rev.pitch_draft_id, rev.scene_hash, 0, now() - INTERVAL '3 minutes'
  FROM consent_revisions rev WHERE rev.id = '${IDS.revA}';
INSERT INTO media_render_jobs (id, revision_id, campaign_id, pitch_draft_id, scene_hash, attempts, created_at)
SELECT 'ee3a4000-0000-4000-8000-000000000002', rev.id, '${IDS.campaignB}', rev.pitch_draft_id, repeat('0', 64), 2, now() - INTERVAL '2 minutes'
  FROM consent_revisions rev WHERE rev.id = '${IDS.revB}';
INSERT INTO media_render_jobs (id, revision_id, campaign_id, pitch_draft_id, scene_hash, attempts, created_at)
SELECT 'ee3a4000-0000-4000-8000-000000000003', rev.id, '${IDS.campaignC}', rev.pitch_draft_id, rev.scene_hash, 2, now() - INTERVAL '1 minute'
    ...
    for (let pass = 0; pass < 5; pass += 1) {
      ...
      const remaining = psql(
        `SELECT count(*) FROM media_render_jobs
          WHERE id IN ('${IDS.jobA}', '${IDS.jobB}', '${IDS.jobC}')
