...
솔로 개발과 8주 성장 기간을 전제로 한 기본안이며 iOS를 먼저 출시해 범위를 줄입니다. 아래 표는 목표 스택이고, 현재 구현 상태는 `docs/SESSION_HANDOFF.md`가 기록합니다 — 특히 Media renderer(MP4)는 미구현(정적 share kit만 판매), analytics는 외부 공급자 없이 자체 `analytics_events`(outcome은 DB 트리거가 server-authoritative로 기록)로 동작합니다.
