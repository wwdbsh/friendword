...
- **sandbox 산출물 제외**: 0060 이후 `purchase_credit_ledger`·`campaign_entitlements`·`purchase_events`의 sandbox 행은 드릴이 만든 테스트 산출물이며 매출·conversion·refund 비율 계산에서 제외합니다(무료 거래이므로 포함하면 전환율은 부풀고 매출은 0으로 희석됩니다). 성장 지표도 같습니다 — 0060 이후 server-stamped `creator_launch_purchased`·`campaign_pass_purchased` 이벤트는 `properties.environment`를 싣고, `scripts/export-growth-evidence.mjs`가 매출 계열 세 지표에서 sandbox를 뺍니다. 절차는 `docs/OPS.md`의 "RevenueCat sandbox 결제 드릴".
- **제외 필터는 테이블마다 다릅니다(계약이 다르기 때문)**: `purchase_credit_ledger.environment`와 `campaign_entitlements.environment`는 서버가 판정한 라벨이라 NOT NULL이고 CHECK로 두 값(`SANDBOX`|`PRODUCTION`)만 갖습니다 — `environment <> 'SANDBOX'`가 정확합니다. 반면 `purchase_events.environment`(0014)는 **RevenueCat 원문 문자열**이라 nullable이고 대소문자 정규화도 없습니다. 여기에 같은 `<> 'SANDBOX'`를 쓰면 NULL 행(=실제 구매)이 통째로 매출에서 빠집니다. 그래서 이 컬럼의 제외 조건은 항상 **`upper(coalesce(environment, '')) <> 'SANDBOX'`** 로 씁니다(0023의 production gate와 같은 형태). 값이 비었거나 낯설면 sandbox가 아니라 **실제 돈으로 셉니다** — 매출 숫자에서 안전한 방향은 과소가 아니라 과대 계상 쪽입니다. PostgREST에는 그 식을 그대로 줄 수 없으므로 `or=(environment.is.null,environment.not.ilike.SANDBOX)`로 같은 술어를 서버측에 보냅니다(동치: NULL·''·`SANDBOXY`는 남고 `SANDBOX`·`sandbox`는 빠집니다).
- **campaign_entitlements 라벨은 강등되지 않습니다**: (campaign, product)당 행이 하나라, 실제 결제 이력이 있는 캠페인에서 나중에 sandbox 드릴을 하면 그 행이 `SANDBOX`로 덮여 실매출이 조용히 집계에서 빠질 수 있습니다. 0060의 upsert는 기존 라벨이 `PRODUCTION`이면 유지합니다. 반대 방향(무료 드릴 행이 PRODUCTION으로 남아 과대 계상)은 눈에 보이고 정정 가능하므로 이쪽을 택했습니다 — 드릴 종료 절차에서 대상 행을 확인합니다.
