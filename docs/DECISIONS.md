...
- **잔여**: hosted `0047` 배포(별도 배포 단계, 웹 배포와 순서 조율 필요 — 마이그레이션만 먼저 가면 구클라이언트의 headline/body 편집이 서버 파생으로 대체되는 창이 생기나 P0 이전과 동일 수준이라 악화는 아님), moderation provider 실호출(OPENAI_API_KEY 사용자 입력 대기), enforcement=on 상태의 브라우저 왕복(로컬 키 부재로 SQL 스위트+해시 대조로 대체 검증됨).
