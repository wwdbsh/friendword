...
### 계정 표면 — 웹·모바일 공통 정보 구조 (확정 2026-08-12, T009)

한 사람의 것은 **허브 하나**에서 시작한다. 웹은 `/me`("My page"), 모바일은
홈 화면의 `My page` 그룹이며, 둘은 **같은 영역에 같은 이름**을 쓴다.

| 영역             | 이름           | 컨트롤이 사는 곳                       |
| ---------------- | -------------- | -------------------------------------- |
| 받은 관심        | Interest inbox | 웹 `/inbox`                            |
| 보낸 관심        | My interests   | 웹 `/interests` · 모바일 `interests`   |
| 인트로룸         | Intro rooms    | 웹 `/rooms`                            |
| 나에 대한 캠페인 | My campaigns   | 웹 `/inbox` · 모바일 `campaigns`       |
| 계정             | Your account   | 웹 `/inbox#account` · 모바일 `account` |

- 허브는 **요약과 디렉터리**다. 카운트·상태·링크만 두고 컨트롤은 옮기지 않는다.
- 셸(`FlowNav`)의 계정 목적지는 **허브 하나**다. 375px에서 셸 폭은 327px이고
  형제 탭 3개만으로 331.7px라 이미 랩됐다 — 목적지가 넷이 된 이상 한 줄에
  들어가는 구성은 허브뿐이다. `aria-current`는 허브에서만 켠다.
- "my page"는 **계정 허브**를 뜻한다. 낯선 사람이 보는 캠페인 페이지는
  **`my public page`**라고 부른다(인박스의 `View my public page`).
- Trust Layer 대역(0~20%): 카운트와 상태가 주인공이고 tilt·bounce는 쓰지 않는다.
- §12: 읽지 못한 영역은 0으로 표시하지 않고 "셀 수 없었다"고 말한다. 허브는
  발신자의 이름·사진·나이를 렌더하지 않는다 — 카운트 정수 하나만 남긴다.
