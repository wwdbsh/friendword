...
> - ~~현재 벤치는 에러 메시지만 반환하고 Chromium stderr·스택을 버린다 → 원격 진단 불가~~ → **T012(Issue #17)로 해소 (2026-08-05)**: 벤치 실패(500) 응답이 이제 실패 단계 마커(`input-synthesis`→`browser-launch`→`page-goto`→`harness-ready`→`payload-load`→`capture`→`encode`→`output-read`), 에러 name·스택(상한), Chromium stderr 테일(16KB 링), 브라우저 exit code/signal, 기동 전 프로브(추출 바이너리 존재·크기·/tmp 가용량), peakMemory를 함께 반환한다. 계측 전용 — 진단 싱크가 없으면 launch 플래그·syscall이 이전과 동일. **남은 절차**: ① T012 병합·배포 → ② 사용자 벤치 1회 재실행 → ③ 500 응답 본문으로 원인 확정(용의: 추출 무결성, headless 모드-바이너리 페어링, 플래그 조합, 실할당 메모리; SIGKILL=OOM vs SIGSEGV=크래시가 exit signal로 갈림) → ④ 수정 태스크 T013 작성·수정·검증·병합 → ⑤ T002 재개. 주의: 함수 전체가 OOM-kill되면 응답 자체가 없다(502) — 그것 자체가 신호.
>
