...
> - **남은 절차**: 사용자가 Pro/Performance 전환 → 배포 → 벤치 재실행(cold 1·warm 1·동시 2) → 그 수치로 T002 판정. **renderMs 판정 기준은 재벤치 후 재협상**(기존 기준은 1vCPU 전제). 함수 전체 OOM-kill이면 응답 없음(502) — 그것 자체가 신호.
>
