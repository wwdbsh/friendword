...
DO $$
BEGIN
  IF to_regprocedure('public.create_dater_revision(uuid,text,text,uuid[])') IS NULL THEN
    RAISE EXCEPTION 'AUDIT2-CP-1: create_dater_revision(uuid,text,text,uuid[]) RPC missing';
  END IF;
